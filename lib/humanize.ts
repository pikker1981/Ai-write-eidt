...
const ruleO1: Rule = {
  ...
  run(text) {
    ...
    const replacements: Array<{ re: RegExp; from: string; to: string; kind: string }> = [
      { re: /(^|[^가-힣])할수([^가-힣]|$)/g, from: "할수", to: "할 수", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])될수([^가-힣]|$)/g, from: "될수", to: "될 수", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])될것([^가-힣]|$)/g, from: "될것", to: "될 것", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])된것([^가-힣]|$)/g, from: "된것", to: "된 것", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])할것([^가-힣]|$)/g, from: "할것", to: "할 것", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])한번([^가-힣]|$)/g, from: "한번", to: "한 번", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])몇가지([^가-힣]|$)/g, from: "몇가지", to: "몇 가지", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])이와같은([^가-힣]|$)/g, from: "이와같은", to: "이와 같은", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])다시한번([^가-힣]|$)/g, from: "다시한번", to: "다시 한 번", kind: "띄어쓰기" },
      { re: /(^|[^가-힣])되요([^가-힣]|$)/g, from: "되요", to: "돼요", kind: "맞춤법" },
      { re: /(^|[^가-힣])왠일([^가-힣]|$)/g, from: "왠일", to: "웬일", kind: "맞춤법" },
      { re: /(^|[^가-힣])않되([^가-힣]|$)/g, from: "않되", to: "안 되", kind: "맞춤법" },
    ];

    for (const { re, from, to, kind } of replacements) {
      t = t.replace(re, (_, pre, post) => {
        found.push(det("O-1", "맞춤법/띄어쓰기", "S1", from, to, `${kind} 자동 교정`));
        return `${pre}${to}${post}`;
      });
    }
